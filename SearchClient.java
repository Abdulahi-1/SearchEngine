...
public class SearchClient {
    ...
    // Behavior: This method mimics a table of contents
    // by relating words to which books they are found in.
    // Words within the documents are considered case-insensitively.
    // Parameters:
    //   - List: contains documents
    // Returns:
    //   - Map: returns a mapping of contents to books that contain the word
    // Exceptions:
    //   - N/A
    ...
    // Behavior: This method allows users to search anything and receive respective
    // book tailored to their search.
    // Parameters:
    //   - index: contains books
    //   - query: the users search
    // Returns:
    //   - Set: returns a set of books that contain the contents of the users search.
    // Exceptions:
    //   - N/A
    public static Set<Media> search(Map<String, Set<Media>> index, String query) {
        Set<Media> queryResults = new TreeSet<>();

        Scanner queryScan = new Scanner(query);

        while(queryScan.hasNext()){
            String queryToken = queryScan.next();
            if(index.containsKey(queryToken)){
                queryResults.addAll(index.get(queryToken));
            }
            return queryResults;
        }
        return queryResults;
